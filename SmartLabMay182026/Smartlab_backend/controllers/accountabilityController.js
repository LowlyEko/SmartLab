  ...
  try {
    ...
    const record = await prisma.accountability.create({
      data: {
        ...
        time_start:             time_start ? new Date(`1970-01-01T${time_start}`) : null,
        time_end:               time_end   ? new Date(`1970-01-01T${time_end}`)   : null,
    ...
    // safeDate throws plain Error for bad dates; Prisma errors have a clientVersion property
...
exports.updateRecord = async (req, res) => {
  try {
    ...
    if (time_start             !== undefined) data.time_start             = time_start ? new Date(`1970-01-01T${time_start}`) : null;
    if (time_end               !== undefined) data.time_end               = time_end   ? new Date(`1970-01-01T${time_end}`)   : null;
