...
window.addEventListener("DOMContentLoaded", async () => {
  // Fetch live stats from the server
  let stats = { total_equipment: 0, pending_requests: 5, damages: 3 };

  try {
    const res = await fetch('api/dashboard-stats.php');
    if (res.ok) {
      const data = await res.json();
      stats.total_equipment   = data.total_equipment   ?? stats.total_equipment;
      // stats.pending_requests = data.pending_requests ?? stats.pending_requests;
      // stats.damages          = data.damages          ?? stats.damages;
    }
  } catch (err) {
  ...
  const counters = [
    { id: "dash-total-equipments", target: stats.total_equipment },
    { id: "dash-pending-requests", target: stats.pending_requests },
    { id: "dash-damages",          target: stats.damages },
  ];
