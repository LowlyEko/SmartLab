...
exports.updateRecord = async (req, res) => {
  try {
    ...
    success(res, { ...record, stage_label: stageLabel(record.email_stage) }, 'Record updated successfully');
  } catch (err) {
...
exports.resolveRecord = async (req, res) => {
  try {
    ...
    success(res, { ...record, stage_label: stageLabel(record.email_stage) }, 'Accountability record resolved successfully');
  } catch (err) {
    if (err.code === 'P2025') return error(res, 'Record not found', 404);
    error(res, err.message);
  }
