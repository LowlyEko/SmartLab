...
async function loadData() {
  ...
  activeRows  = normalised.filter(r => r.dbStatus !== 'Pending');
...
function buildActiveRow(r) {
  const isDanger = r.dbStatus === 'Approved' && isOverdue(r.dateNeeded);
  ...
  document.getElementById('detail-action-btn')?.addEventListener('click', async () => {
    if (type === 'active') {
      ...
      if (idx !== -1) {
        ...
        activeRows[idx].dbStatus = 'RETURNED_LOCAL';
      }
      applyActiveTable();
      close();
...
function buildCardView(section, rows, type) {
  ...
  rows.forEach(row => {
    ...
    if (type === 'active') {
      // FIX: check 'Approved' (not old 'ALLOWED')
      const isDanger = row.dbStatus === 'Approved' && isOverdue(row.dateNeeded);
      card.innerHTML = `
