...
function collectFormPayload() {
  ...
  const equipmentPayload = Array.from(document.querySelectorAll(".equip-chk:checked"))
    ...
    .filter(e => e.equipment_id);  // only items that have a real ID
  ...
  Array.from(document.querySelectorAll("#materials-container .dynamic-row"))
    .forEach(row => {
      ...
      if (typeVal === 'chemicals' || typeVal === 'chemical') {
        // Goes to chemical_items table — backend field is chemical_id
        chemicalsPayload.push({
          chemical_id: parseInt(idVal),
          quantity:    qty,
          remarks:     null,
        });
      } else {
        // apparatus, glassware, equipment, supplies — goes to reservation_equipment
        extraEquipPayload.push({
          equipment_id: parseInt(idVal),
          quantity:     qty,
          remarks:      null,
        });
      }
...
window.viewReservation = function(id) {
  ...
  const equipHtml = r.reservation_equipment?.length
    ? r.reservation_equipment.map(ei => {
        // Look up the real name from loaded inventory
        const found = equipmentItems.find(i => i.id === Number(ei.equipment_id));
        const label = found ? found.name : `Equipment #${ei.equipment_id}`;
        return `
        <li>
