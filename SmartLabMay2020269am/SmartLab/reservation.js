...
async function patchReservationStatus(id, status, note = '') {
  // FIX: new DB uses plain varchar statuses — 'Approved', 'Rejected', 'Pending'
  const body = { status };
...
function normaliseReservation(r) {
  ...
  // time_of_activity is stored as TIME in MySQL — comes back as a string like "08:30:00"
  const timeStart     = r.time_of_activity ? new Date(`1970-01-01T${r.time_of_activity}`) : null;
  ...
  return {
    ...
    conditionsNote:  '',       // not in new schema
    rejectionReason: '',       // not in new schema
    raw:           r
  };
}
...
async function loadData() {
  ...
  // FIX: new DB status strings (not old enums TO_REVIEW / ALLOWED)
...
function attachPendingRowHandlers() {
  ...
  // FIX: send 'Approved' / 'Rejected' (new DB varchar values, not old enums)
  document.querySelectorAll('#pending-tbody .pending-btn').forEach(btn => {
    btn.addEventListener('click', async e => {
      e.stopPropagation();
      const id     = parseInt(btn.dataset.id);
      const action = btn.classList.contains('accept') ? 'Approved' : 'Rejected';
...
function openDetailModal(row, type) {
      ...
      <div class="detail-modal">
        ...
        <div class="detail-modal-body">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value"><span class="id">${escHtml(row.studentId)}</span></span>
          </div>
          <div class="detail-row">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value" style="font-size:13px;">${escHtml(row.equipmentLog)}</span>
          </div>` : ''}
          <div class="detail-row">
          ...
          ${row.timeStart ? `
          <div class="detail-row">
            <span class="detail-label">Time of Activity</span>
            <span class="detail-value">${escHtml(row.timeStart)}</span>
          </div>` : ''}
          <div class="detail-row">
  ...
  // Active: Mark Returned — local-only visual flag (no RETURNED status in new DB)
  document.getElementById('detail-action-btn')?.addEventListener('click', async () => {
    if (type === 'active') {
      ...
      close();
      // FIX: use 'Approved' (not old 'ALLOWED')
  ...
  document.getElementById('detail-reject-btn')?.addEventListener('click', async () => {
    close();
    // FIX: use 'Rejected' (capital R, matches new DB)
...
function getStatusNote(dbStatus, conditionsNote, rejectionReason) {
  // FIX: new DB has no CONDITIONAL status — kept for future-proofing
...
function buildCardView(section, rows, type) {
  ...
  rows.forEach(row => {
    ...
    if (type === 'pending') {
      ...
      // FIX: 'Approved' / 'Rejected' (not old enums)
