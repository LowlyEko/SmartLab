(function () {
  ...
  async function loadRecords() {
    ...
    // Load inventory items for materials-broken autocomplete (non-blocking)
    try {
      const invData = await apiFetch("/inventory");
      inventoryItems = invData.data || invData || [];
      populateInventoryDatalist();
    } catch (err) {
      // silently fail — autocomplete stays empty
    }
  }
  ...
  function populateInventoryDatalist() {
    const names = inventoryItems.map(i => i.name).filter(Boolean);
  ...
  window.openEditModal = function (id) {
    ...
    if (window._matDropdown) window._matDropdown.setValue(r.materialsBroken || ""); else document.getElementById("form-materials-broken").value = r.materialsBroken;
