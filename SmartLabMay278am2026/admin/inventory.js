...
(function () {
  ...
  let borrowedCount = 0; // cached from API
  ...
  async function loadInventory() {
    showTableLoading();
    try {
      const [equipRes, chemRes, locRes, borrowedRes] = await Promise.all([
        apiFetch("/inventory?type=equipment"),
        apiFetch("/inventory?type=chemical"),
        apiFetch("/inventory/locations"),
        apiFetch("/inventory/borrowed-count").catch(() => ({ borrowed_count: 0 })),
      ]);
      ...
      const borrowedData = borrowedRes.data || borrowedRes || {};
  ...
  function normalizeForUI(item) {
    ...
    if (tab === "apparatus" || tab === "glassware") {
      return {
        ...
        brand:       item.brand       || "",
        quantity:    item.total_quantity ?? item.available_quantity ?? 0,
  ...
  const formFields = {
    ...
    chemicals: [
      ...
      { key:"location", label:"Location",   type:"text", placeholder:"e.g. Chem Storage, Acid Cabinet" },
  ...
  function renderStats() {
    ...
    // All tabs: derive Out of Stock from quantity <= 0
    const borrowed = borrowedCount;
    const empty    = rows.filter(r => (r.quantity ?? 0) <= 0).length;
      ...
      document.getElementById("tableBody").innerHTML = paged.map(row => `
        <tr>
          ${cols.map(col => {
            ...
            return `<td>${cellVal !== undefined && cellVal !== "" ? cellVal : "–"}</td>`;
          }).join("")}
        </tr>
      `).join("");
    }
  ...
  window.saveItem = async function () {
    ...
    // Chemicals use a free-text location string, not a location_id FK
    if (!isChemical && !form.location_id) { alert("Location is required."); return; }
  ...
  function buildEquipmentBody(form) {
    return {
      ...
      brand:            form.brand       || "",
      total_quantity:   form.quantity !== undefined && form.quantity !== "" ? parseInt(form.quantity) : undefined,
      location_id:      form.location_id,
      remarks:          form.remarks     || "",
      // equipment-specific
      model:            form.description || form.volume_size || "",
  ...
  function buildChemicalBody(form) {
    return {
      name:           form.name,
      quantity:       form.quantity !== undefined && form.quantity !== "" ? parseInt(form.quantity) : undefined,
      container_size: form.unit     || "",
