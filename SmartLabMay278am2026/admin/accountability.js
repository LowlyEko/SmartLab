(function () {
  ...
  const ARCHIVE_DAYS = 5; // Records with date_replaced older than this move to Transactions page
  ...
  async function loadRecords() {
    try {
      ...
      records = (data.data || []).map(mapRecord).filter(r => {
        // Exclude records already archived to the Transactions page:
        // those with date_replaced set AND older than ARCHIVE_DAYS days
        if (!r.dateReplaced) return true;
  ...
  function renderTable() {
    ...
    if (slice.length === 0) {
      ...
      slice.forEach(r => {
        ...
        const notifyBtn  = row.querySelector(".action-btn.notify");
        ...
        notifyBtn.addEventListener("click", () => window.openNotifyModal(r.id));
  ...
  async function saveRecord() {
    ...
    const payload = {
      ...
      prof_name:              profName,
              prof_email:             profEmail || undefined,
      subject,
  ...
  window.openNotifyModal = function(id) {
    ...
    const stageMap = {
      ...
      student:   'Professor',
      professor: 'Dean',
      dean:      'Resolved — no further notifications needed',
    };
    const nextStageName = stageMap[r.emailStage] || 'Unknown';
    const alreadyDone   = r.emailStage === 'dean' || r.remarks === 'resolved';

    const preview = document.getElementById('notify-stage-preview');
    preview.innerHTML = alreadyDone
      ? `<p style="color:#e53935;">All parties have already been notified. No further emails needed.</p>`
      : `<p>Current stage: <strong>${r.stageLabel}</strong></p>
         <p>Next notification will be sent to: <strong>${nextStageName}</strong></p>`;

    document.getElementById('notify-email').value = r.profEmail || (r._raw && r._raw.prof_email) || '';
    document.getElementById('btn-send-notify').disabled = alreadyDone;

    openModal('notify-modal');
  };

  document.getElementById('btn-send-notify').addEventListener('click', async () => {
    const toEmail = document.getElementById('notify-email').value.trim();
    if (!toEmail) {
      showToast('Please enter a recipient email.', true);
      return;
    }
    ...
    try {
      await apiFetch(`/accountability/${notifyingId}/notify`, {
        method: 'POST',
      ...
      btn.disabled = false;
      btn.innerHTML = `<i class='bx bx-send'></i>&nbsp; Send Notification`;
    }
  });
