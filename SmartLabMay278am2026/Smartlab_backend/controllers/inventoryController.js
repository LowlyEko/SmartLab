...
function normalizeChemical(item) {
  return {
    ...
    unit:               item.container_size  || '',
    volume_size:        item.container_size  || '',
    location_id:        null,
...
exports.deleteEquipment = async (req, res) => {
  try {
    ...
    // Check for linked reservations before deleting
    let linked = 0;
    if (item_type === 'apparatus') {
      linked = await prisma.reservation_apparatus.count({ where: { apparatus_id: id } });
    } else if (item_type === 'glassware') {
      linked = await prisma.reservation_glassware.count({ where: { glassware_id: id } });
    } else if (item_type === 'supplies') {
      linked = await prisma.reservation_supplies.count({ where: { supplies_id: id } });
    } else {
      linked = await prisma.reservation_equipment.count({ where: { equipment_id: id } });
    }

    if (linked > 0) {
      return error(res, 'Cannot delete item linked to existing reservations.', 409);
    }
...
exports.createChemical = async (req, res) => {
  try {
    const { name, quantity, container_size, location, remarks } = req.body;
...
exports.updateChemical = async (req, res) => {
  try {
    ...
    if (container_size !== undefined) data.container_size = container_size || null;
...
/**
 * GET /api/inventory/borrowed-count
 * Returns the total quantity of items currently borrowed.
 * Inventory quantities are decremented on approval, so we sum
 * quantities from junction tables of all Approved reservations.
 * Prisma model names are snake_case matching the schema @@map names.
 */
exports.getBorrowedCount = async (req, res) => {
  try {
    // Find all Approved reservations — no date filter needed since
    // qty is already deducted from inventory tables on approval.
    ...
    if (approvedReservations.length === 0) {
      return success(res, { borrowed_count: 0 });
    }

    const resIds = approvedReservations.map(r => r.reservation_id);

    // Sum quantities from all item junction tables in parallel.
    // Use snake_case model names as defined in the Prisma schema.
    const [appAgg, glassAgg, eqAgg, supAgg] = await Promise.all([
      prisma.reservation_apparatus.aggregate({
        where: { reservation_id: { in: resIds } },
        _sum: { quantity: true },
      }),
      prisma.reservation_glassware.aggregate({
        where: { reservation_id: { in: resIds } },
        _sum: { quantity: true },
      }),
      prisma.reservation_equipment.aggregate({
        where: { reservation_id: { in: resIds } },
        _sum: { quantity: true },
      }),
      prisma.reservation_supplies.aggregate({
        where: { reservation_id: { in: resIds } },
        _sum: { quantity: true },
      }),
    ]);

    const borrowed_count =
      (appAgg._sum.quantity   || 0) +
      (glassAgg._sum.quantity || 0) +
      (eqAgg._sum.quantity    || 0) +
      (supAgg._sum.quantity   || 0);

    success(res, { borrowed_count });
  } catch (err) {
    console.error(err);
    error(res, err.message);
  }
