...
// ===== initPanels() — injects #notifPanel and #msgPanel into document.body =====
function initPanels() {

  // ── 1. Notification Panel ────────────────────────────────────────────────
  if (!document.getElementById('notifPanel')) {
    const notifEl = document.createElement('div');
    notifEl.className = 'notif-panel';
    notifEl.id = 'notifPanel';
    notifEl.innerHTML = `
      ...
      <div class="notif-panel-footer">
        <button onclick="window.location.href='accountability.html'">View all notifications →</button>
      </div>`;
  ...
  if (!document.getElementById('msgPanel')) {
    ...
    msgEl.className = 'msg-panel';
    msgEl.id = 'msgPanel';
    msgEl.innerHTML = `
      <div class="mp-header">
        <div class="mp-header-row">
          <span class="mp-header-title">Messages</span>
          <div class="mp-header-actions">
            <button class="mp-icon-btn" id="optionsBtn" title="Options">
              <i class="fa-solid fa-ellipsis-vertical"></i>
            </button>
            <button class="mp-icon-btn" id="newMsgBtn" title="New message">
        ...
        <div class="mp-search-wrap">
          ...
          <input class="mp-search" id="mpSearch" type="text" placeholder="Search Messenger">
        </div>
        <div class="mp-tabs">
          <button class="mp-tab active" data-tab="all">All</button>
          <button class="mp-tab" data-tab="unread">Unread</button>
          <button class="mp-tab" data-tab="groups">Groups</button>
        </div>
      </div>
      <div class="mp-list" id="mpList">
        <div class="mp-item unread" data-type="dm" data-name="Maria Cruz">
          <div class="mp-avatar">MC</div>
          <div class="mp-info">
            <div class="mp-name">Maria Cruz</div>
            <div class="mp-text">Can I borrow the microscope set for Friday?</div>
          </div>
          <div class="mp-meta"><span class="mp-time">9:41 AM</span><span class="mp-dot"></span></div>
        </div>
        <div class="mp-item unread" data-type="dm" data-name="Rodel Lim">
          <div class="mp-avatar mp-yellow">RL</div>
          <div class="mp-info">
            <div class="mp-name">Rodel Lim</div>
            <div class="mp-text">The centrifuge reservation is confirmed.</div>
          </div>
          <div class="mp-meta"><span class="mp-time">8:55 AM</span><span class="mp-dot"></span></div>
        </div>
        <div class="mp-item" data-type="dm" data-name="Ana Jimenez">
          <div class="mp-avatar mp-red">AJ</div>
          <div class="mp-info">
            <div class="mp-name">Ana Jimenez</div>
            <div class="mp-text">Damage report for Beaker #4 submitted.</div>
          </div>
          <div class="mp-meta"><span class="mp-time">Yesterday</span></div>
        </div>
        <div class="mp-item" data-type="dm" data-name="Karl Buenaventura">
          <div class="mp-avatar">KB</div>
          <div class="mp-info">
            <div class="mp-name">Karl Buenaventura</div>
            <div class="mp-text">Equipment return logged successfully.</div>
          </div>
          <div class="mp-meta"><span class="mp-time">May 1</span></div>
        </div>
        <div class="mp-item" data-type="group" data-name="CAS Lab Team">
          <div class="mp-avatar mp-blue">CL</div>
          <div class="mp-info">
            <div class="mp-name">CAS Lab Team</div>
            <div class="mp-text">Juan: Schedule for next week is posted.</div>
          </div>
          <div class="mp-meta"><span class="mp-time">Apr 30</span></div>
        </div>
        <div class="mp-item unread" data-type="group" data-name="Reservation Alerts">
          <div class="mp-avatar mp-yellow">RA</div>
          <div class="mp-info">
            <div class="mp-name">Reservation Alerts</div>
            <div class="mp-text">3 new reservations pending approval today.</div>
          </div>
          <div class="mp-meta"><span class="mp-time">Apr 29</span><span class="mp-dot"></span></div>
        </div>
        <div class="mp-item" data-type="community" data-name="Science Department">
          <div class="mp-avatar mp-red">SD</div>
          <div class="mp-info">
            <div class="mp-name">Science Department</div>
            <div class="mp-text">Reminder: Annual inventory check on May 10.</div>
          </div>
          <div class="mp-meta"><span class="mp-time">Apr 28</span></div>
        </div>
        <div class="mp-no-results" id="mpNoResults">No messages found.</div>
      </div>
      <div class="mp-footer">
        <button id="viewAllBtn">View all messages →</button>
      </div>`;
    document.body.appendChild(msgEl);
  }

  // ── 3. Position panels (fixed, above header stacking context) ────────────
  const notifPanel = document.getElementById('notifPanel');
  notifPanel.style.cssText = 'position:fixed;top:70px;right:110px;z-index:9999;';

  const msgPanelEl = document.getElementById('msgPanel');
  msgPanelEl.style.cssText = 'position:fixed;top:70px;right:60px;z-index:9999;';
  msgPanelEl.addEventListener('click', e => e.stopPropagation());

  // ── 4. Profile panel (already in HTML, just reposition) ──────────────────
  const profilePanelEl = document.getElementById('profilePanel');
  if (profilePanelEl) {
    document.body.appendChild(profilePanelEl);
    profilePanelEl.style.cssText = 'position:fixed;top:70px;right:10px;z-index:9999;';
    profilePanelEl.addEventListener('click', e => e.stopPropagation());
  }

  // ── 5. Wire up profile panel toggle ──────────────────────────────────────
  const profilePanel = document.getElementById('profilePanel');
  document.querySelector('.profile')?.addEventListener('click', (e) => {
    e.stopPropagation();
    profilePanel?.classList.toggle('open');
    msgPanelEl?.classList.remove('open');
  });
  document.addEventListener('click', () => profilePanel?.classList.remove('open'));

  // ── 6. Wire up messages panel ─────────────────────────────────────────────
  wireMsgPanel();

  // ── 7. Wire up notification bell ─────────────────────────────────────────
  initNotifPanel();
}

// ===== Messages Panel wiring (called after panel is injected) =====
function wireMsgPanel() {
  ...
  function filterMessages() {
    const items = document.querySelectorAll('#msgPanel .mp-item[data-type]');
  ...
  // Initial badge count from unread items
  const initialUnread = document.querySelectorAll('#msgPanel .mp-item.unread').length;
  if (msgBadge) {
    msgBadge.textContent = initialUnread;
    msgBadge.style.display = initialUnread === 0 ? 'none' : 'flex';
  }

  // Wire mp-item click handlers (delegated so they work on dynamically injected items)
  document.getElementById('msgPanel')?.addEventListener('click', e => {
    const item = e.target.closest('.mp-item[data-name]');
    if (!item) return;
    e.stopPropagation();
...
function openChatFromItem(name, initials) {
  ...
  if (mpItem) {
    ...
    if (dot) dot.remove();
    const unreadCount = document.querySelectorAll('#msgPanel .mp-item.unread').length;
    if (msgBadge) {
...
// ===== Notification Bell (called from initPanels) =====
function initNotifPanel() {
  ...
  function makeItem({ icon, color, text, time, href, key }) {
    const item = document.createElement('div');
    item.dataset.notifKey = key || text.replace(/\s+/g, '_').slice(0, 60);
    const readSet = getReadSet();
    const alreadyRead = readSet.has(item.dataset.notifKey);
    item.className = alreadyRead ? 'notif-item' : 'notif-item unread';
    item.innerHTML = `
      <div class="notif-icon ${color}"><i class="${icon}"></i></div>
  ...
  const STORAGE_KEY = 'smartlab_notif_read';
  ...
  function markRead(item) {
    ...
    updateBadge();
    const readSet = getReadSet();
    readSet.add(item.dataset.notifKey);
    saveReadSet(readSet);
  }
  ...
  function renderEmpty() {
    ...
    if (list) list.innerHTML = '<div style="padding:24px;text-align:center;color:#888;font-size:13px;">No new notifications</div>';
    unreadCount = 0;
    updateBadge();
  }
  ...
  async function loadNotifications() {
    const BASE    = (typeof CONFIG !== 'undefined') ? CONFIG.BASE_URL : 'http://localhost:5000/api';
    const headers = (typeof getHeaders === 'function') ? getHeaders() : {};
    const list    = notifPanel.querySelector('.notif-list');
    if (!list) return;

    list.innerHTML = '<div style="padding:24px;text-align:center;color:#888;font-size:13px;"><i class="bx bx-loader-alt bx-spin"></i> Loading…</div>';

    try {
      const [statsRes, reservRes, chemRes] = await Promise.all([
        fetch(`${BASE}/dashboard/stats`,     { headers }),
        fetch(`${BASE}/reservations`,        { headers }),
        fetch(`${BASE}/inventory?type=chemical`, { headers }),
      ]);

      const stats   = statsRes.ok   ? (await statsRes.json()).data   || {} : {};
      const resData = reservRes.ok  ? (await reservRes.json()).data  || [] : [];
      const chemData= chemRes.ok    ? (await chemRes.json()).data    || [] : [];

      const items = [];

      // 1. Pending reservations
      const pending = stats.pendingRequests ?? resData.filter(r => r.status === 'Pending').length;
      if (pending > 0) {
        items.push(makeItem({
          key:   `pending_${pending}`,
          icon:  'bx bxs-hourglass',
          color: 'yellow',
          text:  `<b>${pending} pending reservation${pending > 1 ? 's' : ''}</b> awaiting your approval.`,
          time:  'Now',
          href:  'reservation.html',
        }));
      }

      // 2. Overdue / Invalid reservations
      const overdue = resData.filter(r => r.status === 'Invalid');
      overdue.forEach(r => {
        const name = r.student ? `${r.student.first_name} ${r.student.last_name}` : `RES-${String(r.reservation_id).padStart(3,'0')}`;
        items.push(makeItem({
          key:   `overdue_${r.reservation_id}`,
          icon:  'bx bx-time-five',
          color: 'red',
          text:  `<b>${name}</b> has an overdue reservation (${r.subject || 'item not returned'}).`,
          time:  timeAgo(r.date_borrowed),
          href:  'reservation.html',
        }));
      });

      // 3. Active damages
      const damages = stats.damages ?? 0;
      if (damages > 0) {
        items.push(makeItem({
          key:   `damages_${damages}`,
          icon:  'bx bx-info-circle',
          color: 'red',
          text:  `<b>${damages} unresolved damage report${damages > 1 ? 's' : ''}</b> pending resolution.`,
          time:  'Now',
          href:  'accountability.html',
        }));
      }

      // 4. Low stock chemicals (qty <= 5)
      const LOW_THRESHOLD = 5;
      const lowStock = chemData.filter(c => (c.available_quantity ?? c.quantity ?? 0) <= LOW_THRESHOLD && (c.available_quantity ?? c.quantity ?? 0) > 0);
      lowStock.slice(0, 3).forEach(c => {
        items.push(makeItem({
          key:   `lowstock_${c.id || c.chemical_id || c.name}`,
          icon:  'bx bx-package',
          color: 'blue',
          text:  `<b>Low stock:</b> ${c.name || c.chemical_name} is running low (${c.available_quantity ?? c.quantity} left).`,
          time:  'Now',
          href:  'inventory.html',
        }));
      });

      // 5. Out of stock chemicals
      const outOfStock = chemData.filter(c => (c.available_quantity ?? c.quantity ?? 0) <= 0);
      if (outOfStock.length > 0) {
        items.push(makeItem({
          key:   `outofstock_${outOfStock.length}`,
          icon:  'bx bx-error',
          color: 'red',
          text:  `<b>${outOfStock.length} chemical${outOfStock.length > 1 ? 's' : ''} out of stock</b> — needs restocking.`,
          time:  'Now',
          href:  'inventory.html',
        }));
      }
      ...
      unreadCount = items.filter(i => i.classList.contains('unread')).length;
      updateBadge();
  ...
  document.getElementById('notifMarkAll')?.addEventListener('click', () => {
    ...
    notifPanel.querySelectorAll('.notif-item.unread').forEach(el => {
      ...
      readSet.add(el.dataset.notifKey);
    });
    saveReadSet(readSet);
    unreadCount = 0;
    updateBadge();
...
// ===== Wire newMsgBtn and viewAllBtn after panels are in the DOM =====
function wireSecondaryPanelButtons() {
  // newMsgBtn — injected into msgPanel by initPanels
  document.getElementById('msgPanel')?.addEventListener('click', e => {
    if (e.target.closest('#newMsgBtn')) {
      e.stopPropagation();
      document.getElementById('msgPanel')?.classList.remove('open');
      createComposePopup();
    }
    if (e.target.closest('#viewAllBtn')) {
      ...
      const msgPanel   = document.getElementById('msgPanel');
      const viewAllBtn = document.getElementById('viewAllBtn');
      const expanded   = msgPanel.classList.toggle('expanded');
      if (viewAllBtn) viewAllBtn.textContent = expanded ? '← Collapse messages' : 'View all messages →';
    }
  });
}
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => { initPanels(); wireSecondaryPanelButtons(); });
} else {
  initPanels();
  wireSecondaryPanelButtons();
}