...
function normaliseReservation(r) {
  ...
  // FIX: new DB has equipment_items + chemical_items (not a single 'items' array)
  const itemTags = [
    ...(r.equipment_items || []).map(ei => {
      const name = ei.equipment?.name || 'Equipment';
      return ei.quantity > 1 ? `${name} (x${ei.quantity})` : name;
    }),
    ...(r.chemical_items || []).map(ci => {
      const name = ci.chemical?.name || 'Chemical';
